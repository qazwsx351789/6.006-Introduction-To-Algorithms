...
For each recursion, the level of details would changes from n to n-1.
The level of details would becomes 0 at the buttom of the tree. So the 
...
At $i=0$, there are only three nodes represented three edges. When $0 < i < n$, 
the function SNOWFLAKE-EDGE generates 4 nodes in a round. So there are 
$3 \cdot 4^i$ nodes.}
...
For each node, the function divided a side into 4 line segment, which can make a 
triangle. So the rendering time for a node is $\Theta(1)$. }
...
From Problem (b) we know that there are $3 \cdot 4^i$ nodes at level $i$. So the 
rendering time at level $i$ is $3 \cdot 4^i \times \Theta(1) = \Theta(4^i)$.}
...
Suppose that the cost for the CPU is $T(n)$. The total cost is the 
summantion of the costs for all nodes. So $T(n)=\sum^{n-1}_{i=0} \Theta(4^i) = \Theta(4)+\Theta(4^1)+...+\Theta(4^n)
=\Theta(4^n)$.}
